...
layout = dbc.Col([
    dbc.Row([
        ...
        html.Div(id="tabela-investimento", className="dbc"),
    ]),
    
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='bar-graph-investimentos', style={"margin-right": "20px"}),
        ], width=9),
        ...
        dbc.Col([
            dbc.Card(
                dbc.CardBody([
                    ...
                    html.Legend("R$ -", id="valor_investimentos_card", style={'font-size': '60px'}),
...
# Bar Graph            
@app.callback(
    Output('bar-graph-investimentos', 'figure'),
    [Input('store-investimentos', 'data'),
...
def bar_chart(data, theme):
    ...
    graph = px.bar(df_grouped, x='Categoria', y='Valor', title="investimentos Gerais")
...
# Simple card
@app.callback(
    Output('valor_investimentos_card', 'children'),
    Input('store-investimentos', 'data')
)
