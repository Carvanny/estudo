...
# ========= Layout ========= #
layout = dbc.Card([
                html.H1("My Budget", className="card-title"),
                html.P("Carvanny", className="card-title"),
                html.Hr(),
    ...
    # Seção + NOVO ------------------------
            dbc.Row([
                ...
                dbc.Col([
                    dbc.Button(color="warning", id="open-novo-investimentos",
            ...
            html.Div([
                dbc.Modal([
                    dbc.ModalHeader(dbc.ModalTitle("Adicionar Investimentos")),
                    dbc.ModalBody([
                        dbc.Row([
                            dbc.Col([
                                    dbc.Label("Descrição: "),
                                    dbc.Input(placeholder="Ex.: dividendos da bolsa, herança...", id="txt-investimentos"),
                            ], width=6), 
                            dbc.Col([
                                    dbc.Label("Valor: "),
                                    dbc.Input(placeholder="$100.00", id="valor_investimentos", value="")
                            ], width=6)
                        ]),
                        ...
                        dbc.Row([
                            ...
                            dbc.Col([
                                ...
                                dbc.Checklist(
                                    options=[{"label": "Foi recebida", "value": 1},
                                        {"label": "Investimentos Recorrente", "value": 2}],
                                    value=[1],
                                    id="switches-input-investimentos",
                                    switch=True),
                            ], width=4),

                            dbc.Col([
                                html.Label("Categoria de investimentos"),
                                dbc.Select(id="select_investimentos", options=[{"label": i, "value": i} for i in cat_investimentos], value=cat_investimentos[0])
                            ], width=4)
                        ], style={"margin-top": "25px"}),
                            ...
                            dbc.Accordion([
                                    dbc.AccordionItem(children=[
                                            dbc.Row([
                                                dbc.Col([
                                                    html.Legend("Adicionar categoria", style={'color': 'green'}),
                                                    dbc.Input(type="text", placeholder="Nova categoria...", id="input-add-investimentos", value=""),
                                                    html.Br(),
                                                    dbc.Button("Adicionar", className="btn btn-success", id="add-category-investimentos", style={"margin-top": "20px"}),
                                                    html.Br(),
                                                    html.Div(id="category-div-add-investimentos", style={}),
                                                ], width=6),

                                                dbc.Col([
                                                    html.Legend("Excluir categorias", style={'color': 'red'}),
                                                    dbc.Checklist(
                                                        id="checklist-selected-style-investimentos",
                                                        options=[{"label": i, "value": i} for i in cat_investimentos],
                                                        value=[],
                                                        label_checked_style={"color": "red"},
                                                        input_checked_style={"backgroundColor": "#fa7268",
                                                            "borderColor": "#ea6258"},
                                                    ),                                                            
                                                    dbc.Button("Remover", color="warning", id="remove-category-investimentos", style={"margin-top": "20px"}),
                                                ], width=6)
                                            ]),
                                        ], title="Adicionar/Remover Categorias",
                                    ),
                                ], flush=True, start_collapsed=True, id='accordion-investimentos'),
                                    
                                    html.Div(id="id_teste_investimentos", style={"padding-top": "20px"}),
                                
                                    dbc.ModalFooter([
                                        dbc.Button("Adicionar investimentos", id="salvar_investimentos", color="success"),
                                        dbc.Popover(dbc.PopoverBody("Investimentos Salvo"), target="salvar_investimentos", placement="left", trigger="click"),
                                        ])
                            ], style={"margin-top": "25px"}),
                        ])
                ],
                style={"background-color": "rgba(17, 140, 79, 0.05)"},
                id="modal-novo-investimentos",
                size="lg",
                is_open=False,
                centered=True,
                backdrop=True)
...
# Seção NAV ------------------------
            ...
            dbc.Nav(
                [
                    ...
                    dbc.NavLink("Extratos Investimentos", href="/extratos_investimentos", active="exact"),
...
@app.callback(
    Output("modal-novo-investimentos", "is_open"),
    Input("open-novo-investimentos", "n_clicks"),
    State("modal-novo-investimentos", "is_open")
)
def toggle_modal(n1, is_open):
    if n1:
        return not is_open
...
# Add/Remove categoria investimentos
@app.callback(
    [Output("category-div-add-investimentos", "children"),
    Output("category-div-add-investimentos", "style"),
    Output("select_investimentos", "options"),
    Output('checklist-selected-style-investimentos', 'options'),
    Output('checklist-selected-style-investimentos', 'value'),
    Output('stored-cat-investimentos', 'data')],

    [Input("add-category-investimentos", "n_clicks"),
    Input("remove-category-investimentos", 'n_clicks')],

    [State("input-add-investimentos", "value"),
    State('checklist-selected-style-investimentos', 'value'),
    State('stored-cat-investimentos', 'data')]
)
def add_category(n, n2, txt, check_delete, data):
    cat_investimentos = list(data["Categoria"].values())
    ...
    if n:
        if txt == "" or txt == None:
            txt1 = "O campo de texto não pode estar vazio para o registro de uma nova categoria de investimentos."
            style1 = {'color': 'red'}

    if n and not(txt == "" or txt == None):
        cat_investimentos = cat_investimentos + [txt] if txt not in cat_investimentos else cat_investimentos
        txt1 = f'A categoria {txt} foi adicionada com sucesso!'
        style1 = {'color': 'green'}
    ...
    if n2:
        if check_delete == []:
            ...
            cat_investimentos = [i for i in cat_investimentos if i not in check_delete]  
    
    opt_investimentos = [{"label": i, "value": i} for i in cat_investimentos]
    ...
    data_return = df_cat_investimentos.to_dict()

    return [txt1, style1, opt_investimentos, opt_investimentos, [], data_return]
...
# Enviar Form investimentos
@app.callback(
    Output('store-investimentos', 'data'),

    Input("salvar_investimentos", "n_clicks"),

    [
        State("txt-investimentos", "value"),
        State("valor_investimentos", "value"),
        State("date-investimentos", "date"),
        State("switches-input-investimentos", "value"),
        State("select_investimentos", "value"),
        State('store-investimentos', 'data')
    ]
)
def salve_form_receita(n, descricao, valor, date, switches, categoria, dict_investimentos):
    df_investimentos = pd.DataFrame(dict_investimentos)
